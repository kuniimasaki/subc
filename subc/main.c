...
#          undef BINOP
	    ...
	    case iDECL: {
		// introduces a new local: pops the initial value and conses
		// (symbol . value) onto the current lexical environment, so
		// later iGETGVAR/iSETGVAR of the same symbol in this scope
		// find it via assoc() before falling back to Symbol,value
		oop operand = code[pc++];
		env = newPair(newPair(operand, pop()), env);
		continue;
	    }
	    case iCLOSE: {
		oop func = code[pc++];
		...
		switch (getType(func)) {
			...
			while (parn < parc && argc > 0) {
			    environment = newPair(newPair(get(parv[parn++], Variable,name), pop()), environment);
			    --argc;
			}
			while (parn < parc)
			    environment = newPair(newPair(get(parv[parn++], Variable,name), nil), environment);
			sp += argc;
