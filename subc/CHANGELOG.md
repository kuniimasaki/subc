...
ユーザーからの依頼(「実装済みのものを全部使うようなCコード書いて、実行速度とバグを検証して」)
を受けて作成した統合テストプログラムから、いずれもこの研究テーマの核心に関わる、深刻な
バグを2件発見・修正した。

**バグ1: 構造体のポインタフィールド経由の読み出しでMemory追跡情報が失われる**
(`getMemory()`、ツリーウォーカー・VM共通の既存バグ)

```c
struct Node *dead = head;
head = head->next;   // ← ここ
free(dead);            // "freed memory was not allowed in the heap" で fatal
```
という、ごく普通の連結リスト解放パターンが失敗することに気づいた。`head`自体
(`malloc`で確保した直接のポインタ)は正しく`free`できるのに、`head->next`のように
**構造体のポインタフィールド経由で読み出したポインタは`free`できなかった**。

- **原因**: `getMemory()` の `Tpointer` ケース(構造体メンバ経由でポインタ値を
  読むときに通る)が、フィールドから読んだ生アドレスを、毎回
  `newMemory(value, typeSize(target), 0)` で**真新しい(heapフラグ常に0の)
  `Memory`オブジェクトとして再ラップしていた**。`malloc`/`calloc`/`realloc` が
  作った本来の`Memory`オブジェクト(正しいheap/freeフラグを持つ)とは別物になって
  しまうため、`free()`が「これはheapメモリではない」と(そのラッパー単体を見れば
  正しく)拒否していた。
- **より深刻な影響**: 単に`free()`できないだけでなく、**同じフィールドを2回読むと、
  実体は同じメモリでも追跡上は別々の`Memory`オブジェクトになる**ため、片方の
  読み出し経由で`free()`しても、もう片方の読み出し経由でのアクセスは
  use-after-freeとして検出されない、という検出漏れを引き起こしていた
  (実際に「同じフィールドを2回読み、片方で解放しもう片方でアクセスする」
  テストで確認: 修正前は検出漏れ、修正後は正しく`"getting freed memory was
  not allowed"`で検出)。
- **修正**: `malloc`/`calloc`/`realloc` は割り当てを必ずグローバルな`heap`リストに
  登録しているので、`getMemory()`のTpointerケースで生アドレスをラップする前に、
  まず`heap`リストから同じ`.base`を持つ既存の`Memory`オブジェクトを探す
  `findHeapMemory()`を新設し、見つかればそれを再利用する(見つからなければ
  従来通り新規ラップにフォールバック)ように修正。ツリーウォーカー・VM共通の
  コードなので両方に同時に効く。
- `make test`/`make testvm` → 74 passed, 0 failed(無回帰)。

**バグ2: `-O`固有の、まれでタイミング依存の`SIGSEGV`**
(`execute()`のスタック/フレーム配列がGC回収のタイミングによっては壊れる)

`kitchen-sink.c`のようにトップレベルの`extern`宣言・関数定義が多いプログラムを
`-O`で実行すると、構文解析中(`List_append`→`GC_realloc`)に確率的にクラッシュ
することがあった。最小再現ケースを絞り込んだところ15/15回、フルの
`kitchen-sink.c`では30回中26回という高頻度で再現する一方、AddressSanitizerビルドや
一部の切り分けバージョンでは全く再現しないという、非常にタイミング依存性の高い
挙動だった。

- **調査**: GDB/Valgrindで追跡すると、クラッシュ自体は`List_append`が
  `GC_realloc`に渡すアドレスが`0x20`(明らかに不正、mallocもfreeもされていない)
  になっているために起きていた。ただしこれは**別の、無関係な`List`オブジェクトが
  既に壊れていた結果が表面化しただけ**で、本当の原因はここではないと判断。
  `-O`は`execute()`をプログラム全体で1回ではなく、**トップレベルの宣言・定義
  1つごとに個別に呼び出す**(Task 3以来の設計、`extern`宣言1つ・関数定義1つが
  それぞれ独立した`compile()`+`execute()`のペア)。この`execute()`呼び出しのたびに
  `stack`/`frames`/`envStack`(いずれも`MALLOC`で確保するローカル変数)が新規に
  確保され、`execute()`が戻ると同時に(参照が無くなるので)ガベージになっていた。
  この「使い捨てのGC_malloc」をトップレベル宣言の数だけ繰り返すことが、
  保守的(conservative)ガベージコレクタであるBoehm GCに対し、まだ生きている
  はずの何かを誤って回収・再利用させてしまう機会を与えていたとみられる
  (再現率が環境やビルド方法で大きく揺れるのは、conservative GCの偽陰性に
  典型的な挙動)。
- **修正**: `stack`/`frames`/`envStack`とその容量変数を`static`にし、
  バッファ自体を`execute()`の呼び出しをまたいで永続化する(`sp`/`fp`/`esp`の
  インデックスだけを呼び出しごとにリセットする)ように変更。インタプリタ内の
  関数呼び出し・再帰は全て同一`execute()`呼び出し内のバイトコードレベルの
  `iCALL`/`frames[]`で処理されるため、`execute()`自身が再入することは無く、
  `static`化しても安全。
- **確認**: 修正前は15/15・26/30で再現していた最小ケース・フルケースの両方が、
  修正後は合計75回の実行(20回・30回・25回の3バッチ)で0回に。副作用として
  トップレベル宣言のたびに発生していたmalloc/free churnも無くなり、わずかに
  効率も向上。`nfib(8)=67`・`depth(5000)%256=136`(動的スタック伸長)も
  引き続き正しく動作することを確認、`make test`/`make testvm` → 74 passed,
  0 failed(無回帰)。

**`mydemo/kitchen-sink.c`(新設)、`make kitchensink`/`make kitchensinkvm`**
- 制御構文・再帰・構造体・配列・ポインタ演算・ヒープ(malloc/calloc/realloc/
  free+小さな連結リスト)・`string.h`・`stdio.h`(`sprintf`/`snprintf`含む)・
  `stdlib.h`・`math.h`・`ctype.h`・複合代入・int/long/pointerキャストを1本の
  プログラムで横断的に使う統合テストとして追加(上記2件のバグはいずれもこれを
  書く過程で発見)。ツリーウォーカー・VMで出力が完全一致することを確認。
- **簡易ベンチマーク**: ツリーウォーカー 0.10s(user)、VM 0.07s(user) —
  再帰(`fib(10)`)を含む小規模な多機能プログラムでもVMがわずかに高速。
- この言語の既存の制約(`(int)floatvar`のような数値系統をまたぐキャストは
  `converter()`に対応表が無く未対応。`mydemo/calc.c`がコメントアウトされた
  コードで既に示していた、このセッション以前からの既知の制限)はコメントで
  明記し、あえて踏まないようにした。
